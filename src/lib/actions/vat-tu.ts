...
export async function updateMaterial(id: string, input: MaterialInput) {
  return runAction(async () => {
    const data = materialSchema.parse(input);
    const supabase = await createClient();
    const { data: updated, error } = await supabase.from('materials').update(data).eq('id', id).select('id').single();
...
export async function deleteMaterial(id: string) {
  return runAction(async () => {
    ...
    const { data: deleted, error } = await supabase.from('materials').delete().eq('id', id).select('id').single();
...
export async function updateWarehouse(id: string, input: WarehouseInput) {
  return runAction(async () => {
    const data = warehouseSchema.parse(input);
    const supabase = await createClient();
    const { data: updated, error } = await supabase.from('warehouses').update(data).eq('id', id).select('id').single();
    if (error || !updated) throw new Error(RLS_DENIED);
    revalidatePath('/vat-tu/kho');
  });
}
...
export async function deleteWarehouse(id: string) {
  return runAction(async () => {
    ...
    const { data: deleted, error } = await supabase.from('warehouses').delete().eq('id', id).select('id').single();
    if (error || !deleted) throw new Error(RLS_DENIED);
    await logAudit({
      action: 'delete',
      module: '/vat-tu',
...
export async function updateSupplier(id: string, input: SupplierInput) {
  return runAction(async () => {
    const data = supplierSchema.parse(input);
    const supabase = await createClient();
    const { data: updated, error } = await supabase.from('suppliers').update(data).eq('id', id).select('id').single();
    if (error || !updated) throw new Error(RLS_DENIED);
    revalidatePath('/vat-tu/nha-cung-cap');
  });
}
...
export async function deleteSupplier(id: string) {
  return runAction(async () => {
    ...
    const { data: deleted, error } = await supabase.from('suppliers').delete().eq('id', id).select('id').single();
    if (error || !deleted) throw new Error(RLS_DENIED);
    await logAudit({
      action: 'delete',
      module: '/vat-tu',
