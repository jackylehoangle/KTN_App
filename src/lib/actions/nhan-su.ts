...
export async function deleteEmployee(id: string) {
  return runAction(async () => {
    ...
    const { data: deleted, error } = await supabase.from('employees').delete().eq('id', id).select('id').single();
    if (error || !deleted) throw new Error(RLS_DENIED);
    await logAudit({
      action: 'delete',
      module: '/nhan-su',
...
export async function submitEmployeeContractForApproval(contractId: string) {
  return runAction(async () => {
    ...
    const { data: updatedContract, error: updateError } = await supabase
      ...
      .select('id')
      .single();
    if (updateError || !updatedContract) throw new Error(RLS_DENIED);
...
export async function requestAccountProvisioning(employeeId: string) {
  return runAction(async () => {
    ...
    const { data: updated, error } = await supabase
      .from('employees')
      .update({ account_status: 'da_yeu_cau' })
      .eq('id', employeeId)
      .select('id')
      .single();
    if (error || !updated) throw new Error(RLS_DENIED);
...
export async function deletePosition(id: string) {
  return runAction(async () => {
    ...
    const { data: deleted, error } = await supabase.from('positions').delete().eq('id', id).select('id').single();
    if (error || !deleted) throw new Error(RLS_DENIED);
    await logAudit({
      action: 'delete',
      module: '/nhan-su',
...
export async function deletePayroll(id: string) {
  return runAction(async () => {
    ...
    const { data: deleted, error } = await supabase.from('payroll').delete().eq('id', id).select('id').single();
    if (error || !deleted) throw new Error(RLS_DENIED);
    await logAudit({
      action: 'delete',
      module: '/nhan-su',
