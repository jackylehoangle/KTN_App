...
export const quotationSchema = z.object({
  ...
  valid_until: z.string().optional().transform((v) => v || null),
  status: z.enum(['draft', 'pending_approval', 'sent', 'accepted', 'rejected']),
  total_amount: z.number().min(0),
  notes: z.string().optional(),
  opportunity_id: z.string().uuid('Chọn cơ hội').optional().or(z.literal('')).transform((v) => v || null),
...
export const solarPackageItemSchema = z.object({
  ...
  material_id: z.string().uuid('Chọn vật tư').optional().or(z.literal('')).transform((v) => v || null),
...
export const productionPlanSchema = z.object({
  code: z.string().optional(),
  name: z.string().min(2, 'Tối thiểu 2 ký tự'),
  planned_start: z.string().optional().transform((v) => v || null),
  planned_end: z.string().optional().transform((v) => v || null),
...
export const productionTaskSchema = z.object({
  ...
  assigned_to: z.string().uuid('Chọn người phụ trách').optional().or(z.literal('')).transform((v) => v || null),
